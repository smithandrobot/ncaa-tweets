...
displayName:$.trim($teamData$$[1]),hashTag:$.trim($teamData$$[2]),shortName:$breaks_team$$[2],stream:$stream$$.full_name+"-curated",mentions:$stream$$.count.total}}}$teamsLoaded$$=true}$colorValues$$inline_127_s$$={HAMP:{color:"#4892D3",name:"Hampton Pirates"},PRIN:{color:"#FF9408",name:"Princeton Tigers"},AKR:{color:"#CAB388",name:"Akron Zips"},SMU:{color:"#B10000",name:"Southern Methodist Mustangs"},OAK:{color:"#9A803E",name:"Oakland Golden Grizzlies"},PITT:{color:"#CEC499",name:"Pittsburgh Panthers"},
...
$data$$18_e$$.target.getData();$self$$.scheduleData=$data$$18_e$$.events.event;$data$$18_e$$=$getGamesForRound$$($self$$.loadedRound);if($streamLoadCount$$>0){$streamLoadCount$$--;$("#scoreboard-loader").hide();$($data$$18_e$$).each(function(){var $data$$inline_170_game$$=$(this)[0];$data$$inline_170_game$$=new GameController($data$$inline_170_game$$,$self$$.teams);$games$$.push($data$$inline_170_game$$);$data$$inline_170_game$$.addEventListener("onTeamSelect",$teamClick$$);$data$$inline_170_game$$.addEventListener("onHashTagClick",
$onHashTagClick$$);if($data$$inline_170_game$$.active&&$self$$.firstActive==null)$self$$.firstActive=$data$$inline_170_game$$});$self$$.firstActive&&$self$$.firstActive.view.position();if($streamLoadCount$$==0){$sortGames$$();$(".schedule-scrollbar").scrollbar({handleHeight:151,arrows:false})}}else $($data$$18_e$$).each(function(){var $data$$=$(this)[0];for(game in $games$$)$games$$[game].id==$data$$.id&&$games$$[game].update($data$$)})});$self$$.loadRound=function loadRound($round$$){$getFeedsForRound$$($round$$);
